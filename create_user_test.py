...
#esta funcion cambia los valores en el parametro "firstName"
def get_user_body(first_name):
    #el diccionario que contiene el cuerpo de solicitud se copia
    current_body = data.user_body.copy()
    #se cambia el valor del parametro firstName
    current_body["firstName"] = first_name
    #se devuelve un diccionario con el valor firstName requerido
    return current_body

def positive_assert(first_name):
    #guardar el cuerpo de la solicitud con el nombre Aa
    user_body = get_user_body(first_name)
    #guardar el resultado de la solicitud
    user_response = sender_stand_request.post_new_user(user_body)
    #comprobar el codigo de creacion 201
    assert user_response.status_code == 201
    #comprobar que el authToken no esta vacio
...
# Funcion de prueba negativa para los casos en los que la solicitud devuelve un error relacionado con caracteres
def negative_assert_symbol(first_name):
    # El cuerpo actualizado de la solicitud se guarda en la variable user_body
    user_body = get_user_body(first_name)

    # El resultado se guarda en la variabel user_response
    user_response = sender_stand_request.post_new_user(user_body)

    # Comnprueba si el codigo de estado es 400
...
def test_create_user_number_type_fist_name_get_error_response():
    # El cuerpo actualizado de la solicitud se guarda en la variable user_body
